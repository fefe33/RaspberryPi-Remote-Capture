...
class TCPserver:
    ...
    def requestHandler(self, request, client_sock, addr):
        ...
        if request['cmd'] == 'capture':
            ...
            command = ['sudo', 'tcpdump', '-c', str(request['limit'][0]), '-H']
            try:
                ...
                pass
            # if request['download'] is true, add the -w remote.pcap flag
            try:
                if request['download']:
                    command = command + ['-w', 'remote.pcap']
                else:
                    pass
            except:
                pass
            ...
            # decode each value in the object and serialize
            out['put'] = out['put'].decode('utf-8')
            out['err'] = out['err'].decode('utf-8')
            r = bytes(json.dumps(out), 'utf-8')
            # if the optional 'download' doesnt exist or exists and is set to false
            try:
                ...
                # if it fails to find the download key or the value is set to false, return the current response object as is
                elif request['download'] == False:
                    pass
            except:
                pass

            # print the payload if debug==True
            if self.debug == True:
                print('payload: ', repr(r))
            # open a new socket not related to self.sock
            ...
            # if the treat all hosts as online key is present in the request keys, -Pn option is not present in CMD, and its value in the request is True
            if 'all_online' in list(request.keys()) and '-Pn' not in cmd and request['all_online'] == True:
                cmd = cmd + ['-Pn']
            # run the command
...
p.add_argument('--addr', type=str, nargs=1, help='specifies the host and port the server should listen on [host:port]',
               required=True)
p.add_argument('-d', '--debug', type=str,
               help='prints payload length and payload for use in debugging (use \'on\'/\'off\'')
p.add_argument('-si', '--interface', type=str, nargs=1,
               help='define the scan interface (default=wlan0 [may be different on your device])')
# parse them
args = p.parse_args()
